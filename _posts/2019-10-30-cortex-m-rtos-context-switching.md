...
The core can operate at either a **privileged** or **unprivileged** level.
...
The AAPCS also imposes a requirement that "The stack must be double-word
...
When the FPU is "in use" (CONTROL.FPCA=1), an **extended frame** will be saved
by the hardware:
...
1. Set up a build system and startup sequence for your MCU. Extensive details
...
> WARNING: Over the years I've seen a lot of nasty stack overflows arise here
...
We see the port uses a GCC feature known as **Extended Asm**[^15] to mix C
...
To start **FreeRTOS** and get tasks to run`vTaskStartScheduler` needs to be
...
[^11]:
    [Github FreeRTOS Kernel](https://github.com/FreeRTOS/FreeRTOS-Kernel/tree/master/portable)
...
[^14]:
    [ISB after mrs](https://www.freertos.org/FreeRTOS_Support_Forum_Archive/March_2017/freertos_ISB_isntructions_on_Cortex-M4_port_6fc12825j.html)

[^15]: [Extended Asm](https://gcc.gnu.org/onlinedocs/gcc/Extended-Asm.html)
[^16]:
    [Discussion about DSB in FreeRTOS port](https://www.freertos.org/FreeRTOS_Support_Forum_Archive/May_2017/freertos_Use_of_barriers_when_setting_BASEPRI_on_Cortex_M7_8a85181fj.html)

[^17]:
    [See "Visibility of changes in execution priority resulting from executing an MSR instruction"](https://static.docs.arm.com/ddi0403/eb/DDI0403E_B_armv7m_arm.pdf)
