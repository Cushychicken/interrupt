...
By leveraging these features, exceptions and thread mode code can share the same
...
There are a few ways in software that the "execution priority" can be
...
We'll use SEGGER's JLinkGDBServer[^9] as our debugger to step through these
examples.
...
We can pretty easily compute the **Exception Number** by counting the offset
...
Great! We see the top 3 bits "stuck" which means the NRF52840 MCU supports 8
priority levels (0-7).
...
Let's step through the rest of the code and confirm we see **bkpt 6** followed
by **bkpt 4**:
...
[^1]:
    [MPU
    overview]({% post_url 2019-07-16-fix-bugs-and-secure-firmware-with-the-mpu %})

[^3]:
    [See "Overview of the exceptions supported" section](https://static.docs.arm.com/ddi0403/eb/DDI0403E_B_armv7m_arm.pdf)

[^4]:
    [ARMv7-M Specification](https://static.docs.arm.com/ddi0403/eb/DDI0403E_B_armv7m_arm.pdf)
...
[^10]:
    [See "Lazy context save of FP state" for more details](https://static.docs.arm.com/ddi0403/eb/DDI0403E_B_armv7m_arm.pdf)

[^11]:
    [See "Exception status and control" section](https://static.docs.arm.com/ddi0403/eb/DDI0403E_B_armv7m_arm.pdf)

[^12]:
    [See "GPIO tasks and events" for NRF52 GPIOTE interrupt configuration details](https://infocenter.nordicsemi.com/pdf/nRF52840_PS_v1.1.pdf)
